...
class _FreeFlowScreenState extends State<FreeFlowScreen>
  ...
  Future<void> _goLive(_Vibe vibe) async {
    ...
    try {
      ...
      // v247b — also _log() each step so the on-device DebugPanel
      // surfaces the same trail without needing a Mac.
      _log('info', 'GOLIVE', 'START vibe=${vibe.label} '
          'micGranted=$_micPermissionGranted pcmReady=$_pcmEngineReady');
      // ignore: avoid_print
      ...
      _log('ok', 'AUDIO', 'AudioSession configured');
      // ignore: avoid_print
      ...
      if (!_pcmEngineReady) {
        _log('info', 'PCM', 'engine first-time setup…');
        // ignore: avoid_print
        ...
        _log('info', 'PCM', 'engine already ready — skipping setup');
        // ignore: avoid_print
  ...
  Widget _buildLive() {
    ...
    // Bro v6 b: "the ImHim Looks on roleplay should only show when the
    ...
    return Stack(
      children: [
        ...
        // chrome layout: ImHim Looks wordmark on the left, controls on
        ...
        // ── ImHim Looks wordmark overlay (the "viral back-and-forth" CTA).
        // Bro v5: "add ImHim Looks header on the live roleplay but only when
        ...
        // ImHim Looks wordmark moved INTO the top chrome row above (when
        ...
        // room for the ImHim Looks wordmark that now sits directly above it.
